"""Tests for the PamHandler graph generation pipeline.

Marker conventions
------------------
@pytest.mark.quick        - assumes data is already present in data/pam/;
                            safe to run on every commit.
@pytest.mark.integration  - wipes data/pam/ and regenerates everything
...
from vcker.input_data.pam import PamHandler
...
_EXPECTED_CONFIGS = PamHandler.configs


# ---------------------------------------------------------------------------
# Quick tests - require data/pam/*.clq to already exist
# ---------------------------------------------------------------------------


@pytest.mark.quick
class TestPamHandlerQuick:
    """Fast smoke-tests against already-generated PAM data."""

    def test_download_data_skips_existing(self):
        """download_data() completes without error and finds existing files."""
        handler = PamHandler(force_redownload=False)
    ...
    def test_get_named_graphs_yields_all_instances(self):
        ...
        for name, graph in results:
            assert isinstance(name, str) and name.startswith("pam_graph_")
    ...
    def test_graphs_have_positive_edge_counts(self):
        """Each generated PAM graph has at least one edge."""
        handler = PamHandler(force_redownload=False)
        handler.download_data()
    ...
    def test_generation_is_reproducible(self):
        ...
        h1 = PamHandler(force_redownload=True)
        h1.download_data()

        h2 = PamHandler(force_redownload=True)
        h2.download_data()
...
class TestPamHandlerIntegration:
    """End-to-end tests that wipe and regenerate all PAM data."""

    @pytest.fixture(autouse=True)
    def clean_data_folder(self):
        """Remove the pam data folder before the test."""
        from vcker.input_data.utils import get_data_folder

        pam_dir = get_data_folder() / "pam"
        if pam_dir.exists():
    ...
    def test_full_generation_from_scratch(self):
        """Force-generate all PAM instances and confirm files exist on disk."""
        handler = PamHandler(force_redownload=True)
        handler.download_data()
